# Strategy 4: Phase 0 with Multi-Pass Coarse-to-Fine Useful-Skew Optimization
...
Strategy 4 is a documentation snapshot of the current implementation, including default non-reset Phase 0 and the optional reset experiment.

When this document differs from earlier strategy documents, the source of truth is:

```text
optimizer.cpp > strategy4.md > strategy3.md > strategy2.md
```
...
The implementation is organized into Phase0 plus three main optimization phases:
...
Strategy 4 is the current optimizer implementation: **default non-reset Phase 0 resizes existing buffers using incremental timing updates, Phase 1A uses coarse batched insertion, Phase 1B uses fine greedy insertion with incremental timing updates, and Phase 2 uses multi-pass pattern-based area recovery.**
...
When enabled, this verification flag checks Phase0 accepted incremental moves and Phase1B/Phase2 accepted moves and snapshots against full timing.

The report contains:

- run metadata, constraints, status, legality, violations, modifications, and score
- a compact Baseline / After Phase0 / Phase1A / Phase1B / Phase2 table
...
```text
original tree -> Phase0 -> Phase1A -> Phase1B -> Phase2
```
...
```text
normal                  : original tree -> Phase0 -> Phase1A -> Phase1B -> Phase2
reset_phase0_experiment : fastest-compatible reset -> Phase0 -> Phase1A -> Phase1B -> Phase2
```
...
The normal report includes Baseline, After Phase0, Phase1A, Phase1B, and Phase2. It records Phase 0 settings/counters, candidate count mode, ranked/scanned candidates, early-stop reason, timing mode, verification status, average trial time, batch auto reason/counters, and the Phase1A stop reason. The reset experiment additionally writes `reset_phase0_experiment` and `phase0_compare` reports.
