# Strategy 5: Phase 0 with Repair/Reclaim Cycles

## Purpose

This document describes the **current implementation of `Optimizer::optimize` in `optimizer.cpp`**.

It is intended to replace / supersede `strategy4.md` as the working strategy document for the new post-Phase0 architecture.

Strategy 5 is a documentation snapshot of the current implementation, including default non-reset Phase 0, optional reset experiment, the default Repair/Reclaim cycle stage after Phase0, and Final Alternating Greedy as the default final cleanup.

When this document differs from earlier strategy documents, the source of truth is:

```text
optimizer.cpp > strategy5.md > strategy4.md > strategy3.md > strategy2.md
```
...
```text
Phase 0 : Default non-reset buffer resizing with incremental timing
Cycle   : Phase1A-pulse + pressure-guided full-tree reclaim
Final   : Final Alternating Greedy by default
Legacy  : Phase1B + Phase2 when ENABLE_FINAL_ALTERNATING_GREEDY=OFF
```
...
The current implementation is still partly oracle-driven, but not every trial uses a full `analyze_timing(...)` call:

- Phase 0 uses incremental clock-arrival / path-group updates by default.
- Repair/Reclaim uses full timing for Phase1A-pulse insertion acceptance and incremental timing for reclaim trials.
- Phase 1A uses full timing analysis for each batch.
- Final Alternating Greedy uses incremental clock-arrival / path-group updates.
...
returns true if elapsed time exceeds 570 seconds. The timer starts before Phase 0, so Phase0, Phase1A, Phase1B, and Phase2 consume one shared budget.
...
Phase0 captures the original baseline snapshot and sets `baseline.valid = true`. It uses the same `start_time` and global wall-clock budget as Phase1A, Phase1B, and Phase2.
...
With the current default configuration, this fallback is usually skipped because Phase0 initializes the baseline before Phase1A.
...
When `ENABLE_TIMING_VERIFY=ON`, snapshots also run a full timing-cache consistency check. Cached and full SS/FF TNS, WNS, and violation counts are compared using epsilon `1e-6`; a mismatch prints the snapshot context and aborts.
...
With `ENABLE_TIMING_VERIFY=ON`, the cached snapshot timing is immediately verified against a full `analyze_timing(...)` result.
...
When `ENABLE_TIMING_VERIFY=ON`, the accepted cache state is immediately compared with a full timing recomputation.
...
When `ENABLE_TIMING_VERIFY=ON`, every accepted Phase 2 move and each pattern snapshot is checked against a full timing recomputation.
...
These checks compile only when `ENABLE_TIMING_VERIFY=ON`. A timing mismatch greater than `1e-6`, or any violation-count mismatch, prints cached/full values with the move or snapshot context and aborts.
...
When disabled, none of those debug or report operations run. Expensive full timing-cache verification is controlled separately:

```bash
cmake .. -DENABLE_TIMING_VERIFY=ON
```

When enabled, this verification flag checks Phase0 accepted incremental moves, Final Alternating Greedy accepted moves, and legacy Phase1B/Phase2 accepted moves and snapshots against full timing.
...
The legacy final Phase1B plus Phase2 cleanup can still be selected with `ENABLE_FINAL_ALTERNATING_GREEDY=OFF`.

The optional reset-based comparison experiment is enabled with:

```bash
cmake .. -DENABLE_PHASE0_EXPERIMENT=ON
```
...
At most three candidate types are tested per node, for at most two passes and 90 seconds. Phase 0 uses the same incremental clock-arrival/path-group timing cache as the later phases by default, so type-change trials update only the changed subtree and affected timing groups. A full-timing fallback remains available through `enable_phase0_incremental_timing=false`, and debug full-timing verification can be enabled with `ENABLE_TIMING_VERIFY=ON`.
...
The normal report includes Baseline, After Phase0, RepairReclaimCycles, and FinalAlternatingGreedy by default. `ENABLE_FINAL_ALTERNATING_GREEDY=OFF` restores separate Phase1B and Phase2 rows. It records Phase 0 settings/counters, candidate count mode, ranked/scanned candidates, early-stop reason, timing mode, verification status, average trial time, batch auto reason/counters, Repair/Reclaim cycle counters, runtime/validation optimization diagnostics, and the Phase1A/cycle stop reason. The reset experiment additionally writes `reset_phase0_experiment` and `phase0_compare` reports.

---

# 25. Strategy 5 Repair/Reclaim Cycle Stage

Strategy 5 adds a configurable post-Phase0 architecture:
...
The old final Phase1B plus Phase2 cleanup is still available with `ENABLE_FINAL_ALTERNATING_GREEDY=OFF`.
...
After that, Final Alternating Greedy runs as the default final repair/reclaim stage. With `ENABLE_FINAL_ALTERNATING_GREEDY=OFF`, the legacy Phase1B and Phase2 stages run instead.
...
Strategy 5 uses Final Alternating Greedy as the default final-stage cleanup:
...
It is enabled by default. To recover the old final Phase1B plus Phase2 cleanup, configure with:

```bash
cmake -S . -B build -DENABLE_FINAL_ALTERNATING_GREEDY=OFF
cmake --build build
```
...
final_alt_repair_insertions_per_iter = 100;
...
The timing report includes a `Final Alternating Greedy` section with settings, runtime, stop reason, repair insertions, reclaim accepts, NEW_BUF deletes, resizes, area before/after/saved, and SS/FF timing before/after. In the stage contribution table, default replacement mode prints one `FinalAlternatingGreedy` row instead of separate `Phase1B` and `Phase2` rows.
